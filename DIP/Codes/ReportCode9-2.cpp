...
int main() {
	...
	//Loads Image
	Mat img = imread("360.jpg", CV_LOAD_IMAGE_UNCHANGED);
	...
	*/
	...
	Mat img_original = imread("stuff_color.jpg", CV_LOAD_IMAGE_UNCHANGED);
	resize(img_original, img_original, Size(img_original.cols / 2, img_original.rows / 2), 0, 0, CV_INTER_NN);

	Mat img_subtract_result = img_original - img_360_1time;
